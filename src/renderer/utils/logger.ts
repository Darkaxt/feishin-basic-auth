    ...
    private initializeLoggers(level: LogLevel) {
        const withDebounce = (logLevel: LogSeverity): LogFn => {
            return (message?: any, meta?: any) => {
                const sanitizedMessage = sanitizeForDiagnostics(message);
                const sanitizedMeta = sanitizeForDiagnostics(meta);
                const key = JSON.stringify([logLevel, sanitizedMessage, sanitizedMeta]);
                const now = Date.now();
